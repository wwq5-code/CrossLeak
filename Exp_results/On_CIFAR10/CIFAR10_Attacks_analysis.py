...
def save_tables(rows):
    ...
    with (TABLE_DIR / "parsed_cifar10_attack_results.csv").open("w", encoding="utf-8", newline="") as f:
...
def plot_utility(rows, output_prefix="CIFAR10_Attacks"):
...
def plot_mia(rows, output_prefix="CIFAR10_Attacks"):
    fig, ax = plt.subplots(figsize=(8.0, 5.4))
    for method in METHOD_ORDER:
        marker, linestyle = METHOD_STYLES[method]
        method_rows = rows_for_method(rows, method)
...
def plot_label_inference(rows, output_prefix="CIFAR10_Attacks"):
    metrics = [
...
def plot_precision_recall(rows, output_prefix="CIFAR10_Attacks"):
    metrics = [
        ("delta_z", "CrossLeak"),
...
def plot_feature_overlap(rows, output_prefix="CIFAR10_Attacks"):
...
def plot_feature_overlap_bar(rows, output_prefix="CIFAR10_Attacks"):
    metrics = [
...
def plot_feature_and_clip(rows, output_prefix="CIFAR10_Attacks"):
    ...
    axes[0].set_xlabel("Deleted Class Range")
    ...
    axes[1].set_xlabel("Deleted Class Range")
    ...
    axes[2].set_xlabel("Deleted Class Range")
...
def print_summary(rows):
    print("CIFAR-10 attack analysis parsed from results.txt")
    for method in METHOD_ORDER:
        method_rows = rows_for_method(rows, method)
