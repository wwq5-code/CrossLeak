...
OUTPUT_PREFIX = "TinyImageNet_Attacks"
TABLE_NAME = "parsed_tinyimagenet_attack_results.csv"
...
def plot_label_inference(rows, output_prefix=OUTPUT_PREFIX):
    ...
    if any("ulia_known_k_overlap" in row for row in rows):
        metrics.append(("ulia_known_k_overlap", "ULIA known-k"))
    if any("ulia_dynamic_overlap" in row for row in rows):
        metrics.append(("ulia_dynamic_overlap", "ULIA dynamic"))
...
def plot_precision_recall(rows, output_prefix=OUTPUT_PREFIX):
    ...
    if any("ulia_known_k_precision" in row for row in rows):
        metrics.append(("ulia_known_k", "ULIA known-k"))
    if any("ulia_dynamic_precision" in row for row in rows):
        metrics.append(("ulia_dynamic", "ULIA dynamic"))
