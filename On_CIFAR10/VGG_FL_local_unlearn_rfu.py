...
def federated_unlearning_one_round(global_model, train_set, client_subsets, args, base_seed, round_idx):
    ...
    for cid, client_subset in enumerate(client_subsets):
        ...
        if cid not in chosen_ids:
            ...
            unlearning_steps = max(1, int(normal_steps * float(getattr(args, "unlearning_step_fraction", 0.25))))
            ...
            local_model = prepare_unlearning(erase_loader, remain_loader, local_model, loss_fn, args, unlearning_steps)
...
def configure_args():
    ...
    args.ulia_effective_lr = float(os.environ.get("CIFAR10_RFU_ULIA_Effective_LR", args.unlearning_lr))
    args.ulia_z_threshold = float(os.environ.get("CIFAR10_RFU_ULIA_Z_Threshold", 1.0))
    args.ulia_bias_weight = float(os.environ.get("CIFAR10_RFU_ULIA_Bias_Weight", 1.0))
