...
def args_parser():
    ...
    parser.add_argument('--backdoor_dir', type=str, default='backdoored_cifar100')
...
def get_num_classes_from_dataset(dataset):
    ...
    if hasattr(dataset, "dataset"):
        return get_num_classes_from_dataset(dataset.dataset)
    return 100
...
def _infer_crossleak_features(scored, labels, indices, class_names, args):
    ...
    for feature_id in top_features:
        ...
        top_label_hist = _label_histogram(top_labels, num_classes=get_num_classes(args))
...
def _feature_vote_label_scores(public_feature_info, probe_feature_info, class_names, args):
    ...
    label_scores = {label: 0.0 for label in range(get_num_classes(args))}
    ...
    eval_ks = getattr(args, "crossleak_eval_ks", [1, 3, 5, 10])
    top_k = max(int(getattr(args, "crossleak_top_labels", 3)), max(eval_ks))
    top_labels = _top_class_items(label_scores, class_names, top_k=top_k)
    return label_scores, top_labels


def _label_ranking_metrics(label_scores, true_erased, class_names, ks=(1, 3, 5, 10)):
    true_labels = [int(item["label"]) for item in true_erased.get("top_labels", [])]
    true_set = set(true_labels)
    ranked_labels = [
        int(label)
        for label, _ in sorted(label_scores.items(), key=lambda item: item[1], reverse=True)
    ]
    ranks = {
        label: ranked_labels.index(label) + 1
    ...
    for k in ks:
        k = int(k)
        pred_set = set(ranked_labels[:k])
        metrics[f"hit@{k}"] = bool(true_set.intersection(pred_set)) if true_set else False
...
def _probe_feature_activation_metrics(probe_result):
    if not probe_result or not probe_result.get("top_shifted_features"):
        return {}
    features = probe_result["top_shifted_features"]
    before_features = [f for f in features if f.get("direction") == "before_exclusive_deleted"]
    strengths = [abs(float(f.get("feature_strength", 0.0))) for f in features]
    before_strengths = [abs(float(f.get("feature_strength", 0.0))) for f in before_features]
    before_shifts = [float(f.get("mean_shift", 0.0)) for f in before_features]
    total_strength = sum(strengths)
    return {
        "top_feature_count": int(len(features)),
        "before_exclusive_count": int(len(before_features)),
        "before_exclusive_fraction": float(len(before_features) / len(features)) if features else 0.0,
        "before_exclusive_strength_sum": float(sum(before_strengths)),
        "before_exclusive_strength_ratio": float(sum(before_strengths) / total_strength) if total_strength > 0 else 0.0,
        "before_exclusive_mean_shift": float(np.mean(before_shifts)) if before_shifts else 0.0,
        "top_before_exclusive_strength": float(max(before_strengths)) if before_strengths else 0.0,
    }
...
def _target_label_rank(label_scores, target_label):
    ranked = [
        int(label)
        for label, _ in sorted(label_scores.items(), key=lambda item: item[1], reverse=True)
    ]
    if int(target_label) not in ranked:
        return None
    return int(ranked.index(int(target_label)) + 1)


def _build_backdoor_label_inference_summary(result, args, class_names):
    target_label = int(args.backdoor_target)
    target_name = class_names[target_label] if class_names and target_label < len(class_names) else str(target_label)
    feature_vote_scores = {
        int(label): float(info["score"])
        for label, info in result.get("feature_vote_label_scores", {}).items()
    }
    class_scores = {
        int(label): float(info["score"])
        for label, info in result.get("class_scores", {}).items()
    }
    summary = {
        "target_label": target_label,
        "target_class_name": target_name,
        "feature_vote_target_rank": _target_label_rank(feature_vote_scores, target_label),
        "class_mean_target_rank": _target_label_rank(class_scores, target_label),
        "feature_vote_target_score": float(feature_vote_scores.get(target_label, 0.0)),
        "class_mean_target_score": float(class_scores.get(target_label, 0.0)),
        "feature_vote_top_labels": result.get("feature_vote_predicted_unlearned_labels", []),
        "class_mean_top_labels": result.get("class_mean_predicted_unlearned_labels", []),
        "feature_vote_metrics": result.get("feature_vote_label_metrics", {}),
        "class_mean_metrics": result.get("class_mean_label_metrics", {}),
    }
    return summary


def _normalization_tensors(args, device):
    mean = getattr(args, "norm_mean", (0.5071, 0.4867, 0.4408))
    std = getattr(args, "norm_std", (0.2675, 0.2565, 0.2761))
    mean = torch.tensor(mean, device=device).view(1, -1, 1, 1)
    std = torch.tensor(std, device=device).view(1, -1, 1, 1)
    return mean, std


def _denormalize_images(x, args):
    mean, std = _normalization_tensors(args, x.device)
...
def _trigger_heatmap_score(before_model, after_model, crosscoder, mean, std, x, args):
    before_params = before_model.encoder(x)
    after_params = after_model.encoder(x)
    dim_z = before_params.shape[1] // 2
    h_before = before_params[:, :dim_z]
    h_after = after_params[:, :dim_z]
    hb = ((h_before - mean.to(x.device)) / std.to(x.device)).float()
    ha = ((h_after - mean.to(x.device)) / std.to(x.device)).float()
    z_before = crosscoder.encode_before(hb)
    z_after = crosscoder.encode_after(ha)

    dec_before = crosscoder.decoder_before.weight.T
    dec_after = crosscoder.decoder_after.weight.T
    norm_before = torch.norm(dec_before, dim=1)
    norm_after = torch.norm(dec_after, dim=1)
    rho = norm_before / (norm_before + norm_after + 1e-12)
    exclusivity = torch.abs(rho - 0.5) * 2.0
    feature_mask = exclusivity >= getattr(args, "crossleak_feature_tau", 0.15)
    before_mask = rho >= 0.5
    combined_mask = feature_mask & before_mask
    if int(combined_mask.sum().item()) == 0:
        combined_mask = feature_mask
    if int(combined_mask.sum().item()) == 0:
        top_k = min(getattr(args, "crossleak_top_features", 10), len(exclusivity))
        top_idx = torch.topk(exclusivity, k=top_k).indices
        combined_mask = torch.zeros_like(exclusivity, dtype=torch.bool)
        combined_mask[top_idx] = True

    weights = exclusivity * combined_mask.float()
    latent_shift = torch.abs(z_before - z_after)
    score = (latent_shift * weights.unsqueeze(0)).sum(dim=1)
    return score


def save_backdoor_trigger_heatmaps(before_model, after_model, crosscoder, mean, std, backdoor_dataset, args, seed, output_dir):
    max_images = int(getattr(args, "trigger_heatmap_max_images", 8))
    if max_images <= 0:
        return {}
    ...
    loader = DataLoader(
        backdoor_dataset,
        batch_size=min(max_images, args.batch_size),
        shuffle=False,
        num_workers=1,
    )
    try:
    ...
    x = x[:max_images].to(args.device)
    y = y[:max_images]
    x.requires_grad_(True)
    before_model.eval()
    after_model.eval()
    crosscoder.eval()
    before_model.zero_grad(set_to_none=True)
    after_model.zero_grad(set_to_none=True)
    crosscoder.zero_grad(set_to_none=True)

    score = _trigger_heatmap_score(before_model, after_model, crosscoder, mean, std, x, args)
    score.sum().backward()
    heatmap = x.grad.detach().abs().mean(dim=1, keepdim=True)
    heatmap = _normalize_heatmap(heatmap)

    images = _denormalize_images(x.detach(), args)
    zeros = torch.zeros_like(heatmap)
    heat_rgb = torch.cat([heatmap, zeros, 1.0 - heatmap], dim=1).clamp(0.0, 1.0)
    overlay = (0.55 * images + 0.45 * heat_rgb).clamp(0.0, 1.0)
    grid = torch.cat([images, heat_rgb, overlay], dim=0)

    grid_path = os.path.join(output_dir, f"seed_{seed}_backdoor_trigger_heatmap_grid.png")
    heat_path = os.path.join(output_dir, f"seed_{seed}_backdoor_trigger_heatmaps.png")
    overlay_path = os.path.join(output_dir, f"seed_{seed}_backdoor_trigger_overlays.png")
    torchvision.utils.save_image(grid.cpu(), grid_path, nrow=max_images)
    torchvision.utils.save_image(heat_rgb.cpu(), heat_path, nrow=max_images)
    torchvision.utils.save_image(overlay.cpu(), overlay_path, nrow=max_images)

    return {
        "grid_path": grid_path,
        "heatmap_path": heat_path,
        "overlay_path": overlay_path,
        "num_images": int(x.size(0)),
        "labels": [int(label) for label in y.tolist()],
        "mean_trigger_heatmap_score": float(score.detach().mean().item()),
        "max_trigger_heatmap_score": float(score.detach().max().item()),
    }
...
def _true_erased_label_summary(train_set, client_data_info, args):
    ...
    hist = _label_histogram(labels, num_classes=get_num_classes(args))
...
def _score_extra_probe_dataset(before_model, after_model, crosscoder, mean, std, reference_scored, probe_dataset, args):
    ...
    return {
        ...
        "label_histogram": _label_histogram(acts["labels"].numpy(), num_classes=get_num_classes(args)),
...
def crossleak_attack(before_model, after_model, public_dataset, train_set_for_eval, client_data_info, args, name='model', extra_probe_dataset=None, extra_probe_name='extra_probe'):
    ...
    """
    class_names = getattr(public_dataset, "classes", [str(i) for i in range(get_num_classes(args))])
    ...
    class_scores = {}
    for label in range(get_num_classes(args)):
        mask = labels.numpy() == label
        if np.any(mask):
            class_scores[int(label)] = float(np.mean(scores[mask]))

    eval_ks = tuple(getattr(args, "crossleak_eval_ks", (1, 3, 5, 10)))
    max_eval_k = max(eval_ks)

    class_mean_predicted_unlearned_labels = _top_class_items(
        class_scores,
        class_names,
        top_k=max(int(getattr(args, "crossleak_top_labels", 3)), max_eval_k),
    )
    ...
    class_mean_label_metrics = _label_ranking_metrics(class_scores, true_erased, class_names, ks=eval_ks)
    ...
    feature_vote_label_metrics = _label_ranking_metrics(feature_vote_scores, true_erased, class_names, ks=eval_ks)
    probe_activation_metrics = _probe_feature_activation_metrics(extra_probe_result)
    trigger_heatmap_result = {}
    if extra_probe_dataset is not None and getattr(args, "save_trigger_heatmaps", True):
        heatmap_dir = getattr(args, "trigger_heatmap_dir", None)
        if heatmap_dir is None:
            heatmap_dir = os.path.join(getattr(args, "results_dir", os.getcwd()), "backdoor_trigger_heatmaps")
        trigger_heatmap_result = save_backdoor_trigger_heatmaps(
            before_model,
            after_model,
            crosscoder,
            mean,
            ...
            extra_probe_dataset,
            args,
            seed=getattr(args, "current_seed", 0),
            output_dir=heatmap_dir,
        )

    result = {
        "attacker_uses_erased_data": bool(extra_probe_dataset is not None),
        "relaxed_trigger_visualization_uses_backdoored_data": bool(extra_probe_dataset is not None),
        ...
        "probe_feature_activation_metrics": probe_activation_metrics,
        "top_label_hit": bool(feature_vote_label_metrics.get("hit@3", False)),
        "feature_vote_top_label_hit": bool(feature_vote_label_metrics.get("hit@3", False)),
        "class_mean_top_label_hit": bool(class_mean_label_metrics.get("hit@3", False)),
        "num_public_probe_samples": int(len(labels)),
        extra_probe_name: extra_probe_result,
        "trigger_heatmap_visualization": trigger_heatmap_result,
    }
    result["backdoor_label_inference_summary"] = _build_backdoor_label_inference_summary(result, args, class_names)

    print(f"\nSA-CrossLeak results for {name}:")
    label_summary = result["backdoor_label_inference_summary"]
    print(
        "  BACKDOOR LABEL INFERENCE:",
        f"target={label_summary['target_label']}({label_summary['target_class_name']}), "
        f"feature_vote_rank={label_summary['feature_vote_target_rank']}, "
        f"feature_vote_score={label_summary['feature_vote_target_score']:.4f}, "
        f"class_mean_rank={label_summary['class_mean_target_rank']}, "
        f"class_mean_score={label_summary['class_mean_target_score']:.4f}",
    )
    ...
    print(
        "  feature-vote label metrics:",
        ", ".join(
            [f"hit@{k}={feature_vote_label_metrics.get(f'hit@{k}', False)}" for k in eval_ks]
            + [
                f"best_rank={feature_vote_label_metrics.get('best_true_label_rank')}",
                f"mrr={feature_vote_label_metrics.get('mrr', 0.0):.4f}",
            ]
        ),
    )
    print(
        "  class-mean label metrics:",
        ", ".join(
            [f"hit@{k}={class_mean_label_metrics.get(f'hit@{k}', False)}" for k in eval_ks]
            + [
                f"best_rank={class_mean_label_metrics.get('best_true_label_rank')}",
                f"mrr={class_mean_label_metrics.get('mrr', 0.0):.4f}",
            ]
        ),
    )
    if probe_activation_metrics:
        print(
            "  backdoor_probe deleted-feature activation:",
            f"before_exclusive_fraction={probe_activation_metrics['before_exclusive_fraction']:.4f}, "
            f"strength_ratio={probe_activation_metrics['before_exclusive_strength_ratio']:.4f}, "
            f"strength_sum={probe_activation_metrics['before_exclusive_strength_sum']:.4f}, "
            f"mean_shift={probe_activation_metrics['before_exclusive_mean_shift']:.4f}, "
            f"top_strength={probe_activation_metrics['top_before_exclusive_strength']:.4f}",
        )
    ...
    if trigger_heatmap_result:
        print(
            "  trigger heatmap visualization:",
            f"grid={trigger_heatmap_result['grid_path']}, "
            f"overlay={trigger_heatmap_result['overlay_path']}, "
            f"mean_score={trigger_heatmap_result['mean_trigger_heatmap_score']:.4f}",
        )
...
def configure_backdoor_rfu_args():
    ...
    args.dimZ = 512
    args.batch_size = 200
    args.backdoor_dir = "/home/wwq/PerFU/PerFU/On_CIFAR100/backdoored_cifar100"
    ...
    args.save_trigger_heatmaps = True
    args.trigger_heatmap_max_images = 8
    args.trigger_heatmap_dir = os.path.join(args.results_dir, "backdoor_trigger_heatmaps")
    args.backdoor_result_dir = os.path.join(args.results_dir, "backdoor_crossleak_results")
    args.current_seed = 0
    return args
...
def load_backdoor_data(args, seed):
    ...
    if args.dataset == 'MNIST':
        ...
        if args.dataset == 'CIFAR100':
            ...
            data_root = '/home/wwq/Data/data/cifar'
        ...
        args.norm_mean = norm_mean
        args.norm_std = norm_std
        ...
        train_set = dataset_cls(data_root, train=True, transform=train_transform, download=True)
        test_set = dataset_cls(data_root, train=False, transform=test_transform, download=False)
        train_set_no_aug = dataset_cls(data_root, train=True, transform=test_transform, download=False)
        backdoor_dir = args.backdoor_dir
        if not os.path.isabs(backdoor_dir):
            backdoor_dir = os.path.join(script_dir, backdoor_dir)
        backdoor_dataset = BackdooredImageFolder(
...
def load_backdoored_global_model(args):
    ...
    ckpt_path = resolve_existing_path(
        os.path.join(script_dir, "global_vib_cifar100_fedavg_backdoored.pt"),
        "global_vib_cifar100_fedavg_backdoored.pt",
    )
...
def run_backdoor_rfu_for_seed(args, seed):
    ...
    print(f"\n===== CIFAR100 backdoor RFU seed={seed} =====")
    ...
    print(
        "Loaded backdoored global model metrics: "
        f"seed={seed}, clean_test_acc={clean_acc_before:.4f}, "
        f"backdoor_test_acc={backdoor_acc_before:.4f}"
    )
    ...
    os.makedirs(args.backdoor_result_dir, exist_ok=True)
    seed_result_path = os.path.join(args.backdoor_result_dir, f"backdoor_rfu_crossleak_seed_{seed}.json")
    with open(seed_result_path, "w", encoding="utf-8") as f:
        json.dump(_jsonable(result), f, indent=2)
    print(f"Saved per-seed backdoor CrossLeak result to: {seed_result_path}")
    print(
        ...
        f"feature_vote_hit@1={crossleak_result['feature_vote_label_metrics']['hit@1']}, "
        f"feature_vote_hit@3={crossleak_result['feature_vote_label_metrics']['hit@3']}, "
        f"feature_vote_hit@5={crossleak_result['feature_vote_label_metrics']['hit@5']}, "
        f"feature_vote_hit@10={crossleak_result['feature_vote_label_metrics']['hit@10']}, "
        f"feature_vote_best_rank={crossleak_result['feature_vote_label_metrics']['best_true_label_rank']}, "
        f"class_mean_hit@10={crossleak_result['class_mean_label_metrics']['hit@10']}, "
        f"target_label={crossleak_result['backdoor_label_inference_summary']['target_label']}, "
        f"feature_vote_target_rank={crossleak_result['backdoor_label_inference_summary']['feature_vote_target_rank']}, "
        f"class_mean_target_rank={crossleak_result['backdoor_label_inference_summary']['class_mean_target_rank']}, "
        f"backdoor_probe_score={crossleak_result['backdoor_probe']['mean_score']:.4f}, "
        f"deleted_feature_strength_ratio={crossleak_result['probe_feature_activation_metrics'].get('before_exclusive_strength_ratio', float('nan')):.4f}, "
        f"trigger_heatmap={crossleak_result['trigger_heatmap_visualization'].get('grid_path', '')}, "
...
def summarize_backdoor_results(results):
    metric_getters = {
        ...
        "feature_vote_hit@1": lambda r: float(r["crossleak"]["feature_vote_label_metrics"]["hit@1"]),
        "feature_vote_hit@3": lambda r: float(r["crossleak"]["feature_vote_label_metrics"]["hit@3"]),
        "feature_vote_hit@5": lambda r: float(r["crossleak"]["feature_vote_label_metrics"]["hit@5"]),
        "feature_vote_hit@10": lambda r: float(r["crossleak"]["feature_vote_label_metrics"]["hit@10"]),
        "feature_vote_best_rank": lambda r: (
            np.nan
            if r["crossleak"]["feature_vote_label_metrics"]["best_true_label_rank"] is None
            else r["crossleak"]["feature_vote_label_metrics"]["best_true_label_rank"]
        ),
        "feature_vote_mrr": lambda r: r["crossleak"]["feature_vote_label_metrics"]["mrr"],
        "class_mean_hit@1": lambda r: float(r["crossleak"]["class_mean_label_metrics"]["hit@1"]),
        "class_mean_hit@3": lambda r: float(r["crossleak"]["class_mean_label_metrics"]["hit@3"]),
        "class_mean_hit@5": lambda r: float(r["crossleak"]["class_mean_label_metrics"]["hit@5"]),
        "class_mean_hit@10": lambda r: float(r["crossleak"]["class_mean_label_metrics"]["hit@10"]),
        "class_mean_best_rank": lambda r: (
            np.nan
            if r["crossleak"]["class_mean_label_metrics"]["best_true_label_rank"] is None
            else r["crossleak"]["class_mean_label_metrics"]["best_true_label_rank"]
        ),
        "class_mean_mrr": lambda r: r["crossleak"]["class_mean_label_metrics"]["mrr"],
        "feature_vote_target_rank": lambda r: (
            np.nan
            if r["crossleak"]["backdoor_label_inference_summary"]["feature_vote_target_rank"] is None
            else r["crossleak"]["backdoor_label_inference_summary"]["feature_vote_target_rank"]
        ),
        "class_mean_target_rank": lambda r: (
            np.nan
            if r["crossleak"]["backdoor_label_inference_summary"]["class_mean_target_rank"] is None
            else r["crossleak"]["backdoor_label_inference_summary"]["class_mean_target_rank"]
        ),
        "feature_vote_target_score": lambda r: r["crossleak"]["backdoor_label_inference_summary"]["feature_vote_target_score"],
        "class_mean_target_score": lambda r: r["crossleak"]["backdoor_label_inference_summary"]["class_mean_target_score"],
        ...
        "backdoor_probe_before_exclusive_fraction": lambda r: r["crossleak"]["probe_feature_activation_metrics"].get("before_exclusive_fraction", np.nan),
        "backdoor_probe_before_exclusive_strength_ratio": lambda r: r["crossleak"]["probe_feature_activation_metrics"].get("before_exclusive_strength_ratio", np.nan),
        "backdoor_probe_before_exclusive_strength_sum": lambda r: r["crossleak"]["probe_feature_activation_metrics"].get("before_exclusive_strength_sum", np.nan),
        "backdoor_probe_before_exclusive_mean_shift": lambda r: r["crossleak"]["probe_feature_activation_metrics"].get("before_exclusive_mean_shift", np.nan),
        "backdoor_probe_top_before_exclusive_strength": lambda r: r["crossleak"]["probe_feature_activation_metrics"].get("top_before_exclusive_strength", np.nan),
        "trigger_heatmap_mean_score": lambda r: r["crossleak"]["trigger_heatmap_visualization"].get("mean_trigger_heatmap_score", np.nan),
        "trigger_heatmap_max_score": lambda r: r["crossleak"]["trigger_heatmap_visualization"].get("max_trigger_heatmap_score", np.nan),
...
def print_backdoor_summary(summary, args):
    print("\n===== CIFAR100 backdoor RFU multi-seed mean/std summary =====")
    parts = [
...
def test_backdoor_rfu_multi_seed():
    ...
    print("multi-seed CIFAR100 backdoor RFU evaluation seeds:", args.seeds)
    ...
    os.makedirs(args.backdoor_result_dir, exist_ok=True)
    summary_path = os.path.join(args.backdoor_result_dir, "backdoor_rfu_crossleak_summary.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(_jsonable(summary), f, indent=2)
    print(f"Saved multi-seed backdoor CrossLeak summary to: {summary_path}")
