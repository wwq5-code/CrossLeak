...
def vib_ib_loss_terms(model, x, y, loss_fn, args):
    _, logits_y, _, mu, logvar = model(x, mode='with_reconstruction')
    kld_element = mu.pow(2).add(logvar.exp()).mul(-1).add(1).add(logvar).to(args.device)
    kld_mean = torch.mean(kld_element).mul(-0.5).to(args.device)
    ce_loss = loss_fn(logits_y, y)
    return logits_y, kld_mean, ce_loss


def hutchinson_hessian_trace(model, loss, params):
    ...
    active = [(idx, p, g) for idx, (p, g) in enumerate(zip(params, grads)) if g is not None]
    traces = [None for _ in params]
    if not active:
        return traces

    active_params = [p for _, p, _ in active]
    active_grads = [g for _, _, g in active]
    zs = [
        torch.empty_like(p).bernoulli_(0.5).mul_(2).add_(-1)
    ...
    for (idx, _, _), h_z, z in zip(active, h_zs, zs):
        if h_z is not None:
            traces[idx] = (h_z * z).detach()
    return traces


@torch.no_grad()
def hessian_preconditioned_descent(model, hessian_traces, args):
    damping = getattr(args, "hessian_damping", args.lr)
    hessian_rate = getattr(args, "hessian_rate", args.lr)
    for p, hess in zip([p for p in model.parameters() if p.requires_grad], hessian_traces):
        if p.grad is None:
            continue
        if hess is None:
            denom = torch.ones_like(p).mul_(damping)
        else:
            denom = hess.abs().add(damping)
        p.data.addcdiv_(p.grad, denom, value=-hessian_rate)
...
def prepare_unl(erasing_dataset, dataloader_remaining_after_aux, model, loss_fn, args, noise_flag):
    ...
    for epoch in range(args.local_epochs):
        ...
        for (x_e, y_e), (x_r, y_r) in zip(erasing_dataset, dataloader_remaining_after_aux):
            ...
            _, retain_kld_for_hessian, retain_ce_for_hessian = vib_ib_loss_terms(model, x_r, y_r, loss_fn, args)
            retain_loss_for_hessian = args.beta * retain_kld_for_hessian + retain_ce_for_hessian
            hessian_traces = hutchinson_hessian_trace(model, retain_loss_for_hessian, params)
            model.zero_grad(set_to_none=True)

            logits_y_e, KLD_mean, H_p_q = vib_ib_loss_terms(model, x_e, y_e, loss_fn, args)
            logits_y_r, KLD_mean2, H_p_q2 = vib_ib_loss_terms(model, x_r, y_r, loss_fn, args)
            loss = 0.5*(args.beta * KLD_mean - H_p_q) + 0.5*(args.beta * KLD_mean2 + H_p_q2)

            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5, norm_type=2.0, error_if_nonfinite=False)
            hessian_preconditioned_descent(model, hessian_traces, args)
            model.zero_grad(set_to_none=True)
...
def federated_unlearning_one_round(
    ...
    Returns:
      new_global_model, chosen_client_ids, client_data_info, round_parallel_time, round_communication_cost_bytes
    ...
    round_communication_cost_bytes is total upload + download model traffic.
    """
    if loss_fn is None:
    ...
    model_size_bytes = state_dict_size_bytes(global_sd)
    ...
    round_communication_cost_bytes = 2 * len(client_weights) * model_size_bytes

    return new_global_model, chosen_client_ids, client_data_info, round_parallel_time, round_communication_cost_bytes
...
def configure_mnist_unlearning_args():
    args = args_parser()
    args.gpu = 0
    ...
    args.global_rounds = 5       # round 1 only aggregates unlearning clients
    args.local_epochs = 1         # local epochs per client per round
    args.frac = 1.0               # fraction of clients per round (1.0 = all clients)
    args.seeds = [0, 1, 2, 3, 4]
    args.hessian_rate = 0.001
    args.hessian_damping = args.lr
...
def print_seed_sweep_summary(summary):
    print("\n===== Hessian unlearning multi-seed mean/std summary =====")
    for row in summary:
        parts = [
...
def save_seed_sweep_records(all_seed_results, summary, args):
    ...
    raw_path = os.path.join(args.results_dir, "hessian_multi_seed_raw.json")
    summary_json_path = os.path.join(args.results_dir, "hessian_multi_seed_summary.json")
    summary_csv_path = os.path.join(args.results_dir, "hessian_multi_seed_summary.csv")
...
def run_unlearning_clients_sweep_for_seed(args, seed):
    ...
    for num_unlearn_clients in range(1, 6):
        print(f"\n===== Hessian unlearning test: num_unlearn_clients={num_unlearn_clients} =====")
        ...
        print(
            "Hessian unlearning sweep result: "
    ...
    print(f"\n===== Hessian unlearning sweep summary (seed={seed}) =====")
...
def test_unlearning_clients_sweep():
    args = configure_mnist_unlearning_args()
    print('\n'.join(f'{k}={v}' for k, v in vars(args).items()))
    print("device", args.device)
    print("multi-seed Hessian unlearning evaluation seeds:", args.seeds)
