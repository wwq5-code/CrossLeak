...
def print_seed_sweep_summary(summary):
    print("\n===== OTFU multi-seed mean/std summary =====")
    for row in summary:
        parts = [
...
def save_seed_sweep_records(all_seed_results, summary, args):
    ...
    raw_path = os.path.join(args.results_dir, "otfu_multi_seed_raw.json")
    summary_json_path = os.path.join(args.results_dir, "otfu_multi_seed_summary.json")
    summary_csv_path = os.path.join(args.results_dir, "otfu_multi_seed_summary.csv")
...
def run_unlearning_clients_sweep_for_seed(args, seed):
    ...
    for num_unlearn_clients in range(1, 6):
        print(f"\n===== OTFU test: num_unlearn_clients={num_unlearn_clients} =====")
        ...
        acc_after = eva_vib(
            ...
            test_loader,
            args,
            name=f"after OT fusion unlearning ({num_unlearn_clients} clients)",
...
def test_unlearning_clients_sweep():
    args = configure_mnist_otfu_args()
    print('\n'.join(f'{k}={v}' for k, v in vars(args).items()))
    print("device", args.device)
    print("multi-seed OTFU evaluation seeds:", args.seeds)
