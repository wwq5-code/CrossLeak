...
def configure_mnist_backdoor_fusion_without_ot_args():
    args = args_parser()
    args.gpu = 0
...
def print_seed_sweep_summary(summary):
    print("\n===== MNIST Backdoor FusionWithoutOT multi-seed mean/std summary =====")
    for row in summary:
        parts = [
...
def save_seed_sweep_records(all_seed_results, summary, args):
    ...
    raw_path = os.path.join(args.results_dir, "fusion_without_ot_backdoor_multi_seed_raw.json")
    summary_json_path = os.path.join(args.results_dir, "fusion_without_ot_backdoor_multi_seed_summary.json")
    summary_csv_path = os.path.join(args.results_dir, "fusion_without_ot_backdoor_multi_seed_summary.csv")
...
def run_unlearning_clients_sweep_for_seed(args, seed):
    ...
    print(f"\n===== MNIST Backdoor FusionWithoutOT seed={seed} =====")
    ...
    for num_unlearn_clients in range(1, 6):
        print(f"\n===== MNIST Backdoor FusionWithoutOT test: num_unlearn_clients={num_unlearn_clients} =====")
        ...
        acc_after = eva_vib(
            ...
            test_loader,
            args,
            name=f"after direct fusion backdoor unlearning without OT ({num_unlearn_clients} clients)",
        ...
        backdoor_acc_after = eva_vib(
            ...
            backdoor_loader,
            args,
            name=f"after direct fusion backdoor attack without OT ({num_unlearn_clients} clients)",
            epoch=args.global_rounds,
        )
        ...
        print(
            "MNIST Backdoor FusionWithoutOT sweep result: "
    ...
    print(f"\n===== MNIST Backdoor FusionWithoutOT sweep summary (seed={seed}) =====")
...
def test_unlearning_clients_sweep():
    args = configure_mnist_backdoor_fusion_without_ot_args()
    print('\n'.join(f'{k}={v}' for k, v in vars(args).items()))
    print("device", args.device)
    print("multi-seed MNIST Backdoor FusionWithoutOT evaluation seeds:", args.seeds)
