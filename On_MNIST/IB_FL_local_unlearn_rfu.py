...
def federated_unlearning_one_round(
    ...
    Returns:
      ...
      round_communication_cost_bytes
    ...
    round_communication_cost_bytes is the sum of server-to-client downloads and
    client-to-server uploads for the round.
    """
    if loss_fn is None:
    ...
    model_bytes = state_dict_size_bytes(global_sd)
    download_count = 0
    upload_count = 0
    ...
    round_communication_cost_bytes = (download_count + upload_count) * model_bytes
...
def flatten_result_metrics(result):
    metrics = {
        ...
        "communication_cost_bytes": result.get("communication_cost_bytes", float("nan")),
        "communication_cost_mb": result.get("communication_cost_mb", float("nan")),
    }
...
def print_seed_sweep_summary(summary):
    print("\n===== RFU multi-seed mean/std summary =====")
    for row in summary:
        parts = [
    ...
    for num_unlearn_clients in range(1, 6):
        ...
        for rnd in range(1, args.global_rounds + 1):
            (
                ...
                round_parallel_time,
                round_communication_cost_bytes,
            ) = federated_unlearning_one_round(
...
def test_unlearning_clients_sweep():
    ...
    print("multi-seed RFU evaluation seeds:", args.seeds)
