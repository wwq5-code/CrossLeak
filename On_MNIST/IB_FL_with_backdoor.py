...
def add_trigger_new(add_backdoor, dataset, poison_samples_size, mode, target_label=7, trigger_value=1.0):
    ...
    for i in range(poison_samples_size):
        ...
        if add_backdoor == 1:
            ...
            x_poisoned[:, height - 4:height - 2, width - 4:width - 2] = trigger_value_clamped
    ...
    return (
        ...
        TensorImageIntLabelDataset(poisoned_images.clone(), triggered_clean_labels),
    )


def save_backdoored_samples(backdoor_dataset, output_dir):
    ...
    print(f"Saved {len(backdoor_dataset)} backdoored MNIST images to {output_dir}")
...
if args.dataset == 'MNIST':
    ...
    backdoored_train_set, backdoored_train_set_with_clean_labels = add_trigger_new(
        add_backdoor=1,
        dataset=train_set,
    ...
    backdoor_output_dir = os.path.join(os.path.dirname(__file__), "backdoored_mnist")
    save_backdoored_samples(backdoored_train_set, backdoor_output_dir)
...
## save and test
save_path = "global_vib_mnist_fedavg_backdoored.pt"
torch.save({
