...
def args_parser():
    ...
    parser.add_argument('--dataset', choices=['MNIST', 'CIFAR10', 'CIFAR100', 'tinyIMAGENET'], default='tinyIMAGENET')
...
class VIB(nn.Module):
    def __init__(self, encoder, approximator, decoder, reconstruction_dim=None, use_fc3=True):
        super().__init__()

        self.encoder = encoder
        self.approximator = approximator
        self.decoder = decoder
        image_dim = reconstruction_dim if reconstruction_dim is not None else decoder.fc2.out_features
        self.fc3 = nn.Linear(image_dim, image_dim) if use_fc3 else nn.Identity()
...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        approximator = LinearModel(n_feature=args.dimZ, n_output=200)
    ...
    vib = VIB(encoder, approximator, decoder, reconstruction_dim, use_fc3=(args.dataset != 'tinyIMAGENET'))
    vib.to(args.device)
    return vib, lr
...
class TinyImageNetDataset(Dataset):
    def __init__(self, root_dir, transform=None, train=True):
        ...
        classes = sorted(name for name in os.listdir(train_root) if os.path.isdir(os.path.join(train_root, name)))
        class_to_idx = {cls: idx for idx, cls in enumerate(classes)}

        if self.train:
            for cls in classes:
                cls_dir = os.path.join(train_root, cls, 'images')
...
def state_dict_num_bytes(state_dict):
    """
    Count the communication payload size of a state_dict in bytes.
    """
    total = 0
...
def federated_unlearning_one_round(
    ...
    Returns:
      new_global_model, chosen_client_ids, client_data_info, round_parallel_time, communication_cost
    ...
    communication_cost is the sum of server->client downloads and client->server uploads.
    """
    if loss_fn is None:
    ...
    global_model_bytes = state_dict_num_bytes(global_sd)
    ...
    for cid in all_client_ids:
        ...
        communication_cost["download_bytes"] += global_model_bytes
        ...
        local_sd = copy.deepcopy(local_model.state_dict())
        communication_cost["upload_bytes"] += state_dict_num_bytes(local_sd)
        unlearned_weights.append(local_sd)
        del local_model
    ...
    communication_cost["total_bytes"] = (
        communication_cost["download_bytes"] + communication_cost["upload_bytes"]
    )

    return new_global_model, chosen_client_ids, client_data_info, round_parallel_time, communication_cost
...
def configure_tinyimagenet_fusion_without_ot_args():
    args = args_parser()
    args.gpu = 0
...
def load_tinyimagenet_data(args, seed):
    ...
    train_set = TinyImageNetDataset(root_dir=data_dir, transform=train_transform, train=True)
    test_set = TinyImageNetDataset(root_dir=data_dir, transform=test_transform, train=False)
    train_set_no_aug = TinyImageNetDataset(root_dir=data_dir, transform=test_transform, train=True)
    ...
    args.x_shape = (3, args.image_size, args.image_size)
...
def load_global_tinyimagenet_model(args):
    ...
    ckpt_path = resolve_existing_path(
        os.path.join(script_dir, "global_vib_tinyimagenet_fedavg.pt"),
        "global_vib_tinyimagenet_fedavg.pt",
    )
...
def flatten_result_metrics(result):
    metrics = {
        ...
        "communication_download_bytes": result["communication_download_bytes"],
        "communication_upload_bytes": result["communication_upload_bytes"],
...
def print_seed_sweep_summary(summary):
    print("\n===== TinyImageNet FusionWithoutOT multi-seed mean/std summary =====")
    for row in summary:
        parts = [
...
def run_unlearning_clients_sweep_for_seed(args, seed):
    ...
    print(f"\n===== TinyImageNet FusionWithoutOT seed={seed} =====")

    train_set, train_set_no_aug, test_set, train_loader, test_loader, client_subsets = load_tinyimagenet_data(args, seed)
    global_vib, ckpt = load_global_tinyimagenet_model(args)
    ...
    for num_unlearn_clients in range(1, 6):
        ...
        running_time = 0.0
        communication_download_bytes = 0
        communication_upload_bytes = 0
        client_data_info = {}
        chosen_ids = []
        new_global_vib = run_model
        for rnd in range(1, args.global_rounds + 1):
            new_global_vib, chosen_ids, client_data_info, round_parallel_time, round_communication_cost = federated_unlearning_one_round(
            ...
            communication_download_bytes += round_communication_cost["download_bytes"]
            communication_upload_bytes += round_communication_cost["upload_bytes"]
            print(
                f"Round {rnd} parallel client time: {round_parallel_time:.4f}s, "
                f"communication_cost={round_communication_cost['total_bytes'] / (1024 ** 2):.4f} MB "
                f"(download={round_communication_cost['download_bytes'] / (1024 ** 2):.4f} MB, "
                f"upload={round_communication_cost['upload_bytes'] / (1024 ** 2):.4f} MB)"
            )
        ...
        communication_cost_bytes = communication_download_bytes + communication_upload_bytes
        result = {
            ...
            "communication_download_bytes": communication_download_bytes,
            "communication_upload_bytes": communication_upload_bytes,
        ...
        print(
            ...
            f"(download={communication_download_bytes / (1024 ** 2):.4f} MB, "
            f"upload={communication_upload_bytes / (1024 ** 2):.4f} MB)"
        )

        del run_model, pre_unlearn_vib, new_global_vib
        torch.cuda.empty_cache()

    print(f"\n===== TinyImageNet FusionWithoutOT sweep summary (seed={seed}) =====")
    for result in sweep_results:
        print(
            ...
            f"(download={result['communication_download_bytes'] / (1024 ** 2):.4f} MB, "
            f"upload={result['communication_upload_bytes'] / (1024 ** 2):.4f} MB)"
        )
...
def test_unlearning_clients_sweep():
    args = configure_tinyimagenet_fusion_without_ot_args()
    print('\n'.join(f'{k}={v}' for k, v in vars(args).items()))
    print("device", args.device)
    print("multi-seed TinyImageNet FusionWithoutOT evaluation seeds:", args.seeds)
