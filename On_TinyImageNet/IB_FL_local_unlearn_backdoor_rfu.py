...
def args_parser():
    ...
    parser.add_argument('--backdoor_dir', type=str, default='backdoored_tinyimagenet')
...
class TinyImageNetDataset(Dataset):
    def __init__(self, root_dir, transform=None, train=True):
        ...
        self.classes = sorted(name for name in os.listdir(train_root) if os.path.isdir(os.path.join(train_root, name)))
...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        decoder = LinearModel(n_feature=args.dimZ, n_output=3 * args.image_size * args.image_size)
        lr = args.lr
...
def configure_backdoor_rfu_args():
    ...
    args.batch_size = 200
    args.image_size = 64
    args.backdoor_dir = "/home/wwq/PerFU/PerFU/On_TinyImageNet/backdoored_tinyimagenet"
...
def load_backdoored_global_model(args):
    ...
    ckpt_path = resolve_existing_path(
        os.path.join(script_dir, "global_vib_tinyimagenet_fedavg_backdoor.pt"),
        "global_vib_tinyimagenet_fedavg_backdoor.pt",
    )
...
def run_backdoor_rfu_for_seed(args, seed):
    ...
    print(f"\n===== TinyImageNet backdoor RFU seed={seed} =====")
...
def print_backdoor_summary(summary, args):
    print("\n===== TinyImageNet backdoor RFU multi-seed mean/std summary =====")
    parts = [
...
def test_backdoor_rfu_multi_seed():
    ...
    print("multi-seed TinyImageNet backdoor RFU evaluation seeds:", args.seeds)
