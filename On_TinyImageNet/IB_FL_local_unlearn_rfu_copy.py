...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_y = logits_y.reshape((B, num_classes))  # (B, num_classes)
...
def resolve_unlearning_target_classes(args):
    ...
    if candidates is None:
        candidates = list(range(getattr(args, "num_classes", 200)))
...
def get_num_classes(args=None, dataset=None):
    ...
    if args is not None and getattr(args, "dataset", None) == "tinyIMAGENET":
        return 200
    if args is not None and getattr(args, "dataset", None) == "CIFAR100":
        return 100
    return 10
...
def _true_erased_label_summary(train_set, client_data_info):
    ...
    hist = _label_histogram(labels, num_classes=get_num_classes(dataset=train_set))
...
def _label_set_metrics(predicted_labels, true_erased, args):
    ...
    k = max(1, min(k, get_num_classes(args)))
...
def crossleak_attack(before_model, after_model, public_dataset, train_set_for_eval, client_data_info, args, name='model'):
    ...
    """
    num_classes = get_num_classes(args, public_dataset)
    class_names = getattr(public_dataset, "classes", [str(i) for i in range(num_classes)])
...
def configure_tinyimagenet_unlearning_args():
    args = args_parser()
    args.gpu = 0
...
def print_seed_sweep_summary(summary):
    print("\n===== TinyImageNet RFU multi-seed mean/std summary =====")
    for row in summary:
        parts = [
...
def run_unlearning_clients_sweep_for_seed(args, seed):
    ...
    print(f"\n===== TinyImageNet RFU sweep summary (seed={seed}) =====")
...
def test_unlearning_clients_sweep():
    args = configure_tinyimagenet_unlearning_args()
    print('\n'.join(f'{k}={v}' for k, v in vars(args).items()))
    print(
        "TinyImageNet RFU/CrossLeak config: "
