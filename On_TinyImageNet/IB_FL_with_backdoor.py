...
def args_parser():
    ...
    parser.add_argument('--trigger_margin', type=int, default=4)
...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        model_ft.fc = nn.Linear(num_ftrs, args.dimZ * 2)
        encoder = model_ft   # resnet18(1, 49*2)
...
if args.dataset == 'MNIST':
    ...
    test_set = CIFAR10('/home/wwq/Data/data/cifar', train=False, transform=test_transform, download=True)
    train_set_no_aug = CIFAR10('/home/wwq/Data/data/cifar', train=True, transform=test_transform, download=True)
elif args.dataset == 'CIFAR100':
    ...
    train_set = CIFAR100('/home/wwq/Data/data/cifar100', train=True, transform=train_transform, download=True)
    test_set  = CIFAR100('/home/wwq/Data/data/cifar100', train=False, transform=test_transform, download=True)
    train_set_no_aug = CIFAR100('/home/wwq/Data/data/cifar100', train=True, transform=test_transform, download=True)
    ...
    test_transforms = transforms.Compose([
        transforms.Resize((args.image_size, args.image_size)),  # 调整图像尺寸
        # transforms.RandomHorizontalFlip(),  # 随机水平翻转
        transforms.ToTensor(),  # 转换为 Tensor
        transforms.Normalize(mean=IMAGENET_MEAN,  # 标准化（与预训练模型一致）
                             std=IMAGENET_STD)
    ])
    ...
    train_set = TinyImageNetDataset(root_dir=data_dir, transform=transform, train=True)
    test_set = TinyImageNetDataset(root_dir=data_dir, transform=test_transforms, train=False)
    train_set_no_aug = TinyImageNetDataset(root_dir=data_dir, transform=test_transforms, train=True)
    backdoor_source_set = TinyImageNetDataset(
        root_dir=data_dir,
        train=True,
        transform=transforms.Compose([
            transforms.Resize((args.image_size, args.image_size)),
    ...
    backdoor_output_dir = os.path.join(os.path.dirname(__file__), "backdoored_tinyimagenet")
    save_backdoored_samples(backdoored_train_set_for_save, backdoor_output_dir, dataset_name="TinyImageNet")
...
if args.dataset == 'MNIST':
    ...
    x_shape = (3, args.image_size, args.image_size)
else:
...
save_path = "global_vib_tinyimagenet_fedavg_backdoor.pt"
torch.save({
...
ckpt_path = "global_vib_tinyimagenet_fedavg_backdoor.pt"
